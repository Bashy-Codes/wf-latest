...
export const useCreateDiscussion = () => {
  ...
  const createMutation = useMutation(api.communities.discussions.createDiscussion);
  const updateImageMutation = useMutation(api.communities.discussions.updateDiscussionImage);
  ...
  const confirmCreate = useCallback(async () => {
    ...
    try {
      ...
      Toast.show({
        type: "error",
        text1: "Create Failed",
        text2: "Failed to create discussion. Please try again.",
      });
    }
